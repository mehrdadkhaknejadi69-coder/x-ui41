...
async def create_inbound_client(uid: str, request: Request, _=Depends(require_auth)):
    ...
    async with LINKS_LOCK:
        ...
        if client_limit and existing_clients >= client_limit:
            raise HTTPException(status_code=409, detail=f"ظرفیت اینباند تکمیل است ({client_limit} کاربر)")
    label = str(body.get("label") or f"Client · {existing_clients+1}").strip()[:120]
    limit_bytes = safe_int(body.get("limit_bytes", source.get("limit_bytes", 0)), minimum=0)
    ...
    child_uid, child = await make_link(
        ...
        limit_bytes=limit_bytes,
        expires_at=expires_at,
        note=str(body.get("note") or source.get("note") or "")[:500],
        ...
        ip_limit=safe_int(body.get("ip_limit", source.get("ip_limit", 0)), minimum=0),
        speed_limit_bytes=safe_int(body.get("speed_limit_bytes", source.get("speed_limit_bytes", 0)), minimum=0),
        connection_limit=safe_int(body.get("connection_limit", source.get("connection_limit", 0)), minimum=0),
...
async def subscription_portal(uuid: str, request: Request):
    ...
    bot_username = os.environ.get("TELEGRAM_BOT_USERNAME", "VodiWalkerBot")
    renew_url = f"https://t.me/{bot_username}?start=renew_{uuid[:8]}"
    support_url = f"https://t.me/{str(SUPPORT_USERNAME).lstrip('@')}"
    plan_badge = str(link.get("category_name") or "")
    safe={"label":escape_html(label),"protocol":escape_html(protocol),"raw":escape_html(raw_url),"info":escape_html(info_url),"uuid":escape_html(uuid),"remaining":escape_html(remaining),"expires":escape_html(expires[:19]),"status":"فعال" if active else "غیرفعال","pct":str(pct),"pctclass":pct_class,"used":escape_html(fmt_bytes(used)),"limit":escape_html(fmt_bytes(limit) if limit else "نامحدود"),"ip":str(ip_limit or 0),"conn":str(conn_limit or 0),"days":escape_html(days_text),"daysclass":days_class,"renew":escape_html(renew_url),"support":escape_html(support_url),"plan":escape_html(plan_badge) if plan_badge else ""}
