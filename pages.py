...
body{
  min-height:100vh;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;padding:24px;
  background:
...
.grid-bg{position:absolute;inset:0;opacity:.35;background-image:linear-gradient(rgba(255,255,255,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.045) 1px,transparent 1px);background-size:38px 38px;mask-image:radial-gradient(65% 55% at 50% 30%,#000 20%,transparent 85%)}
 .grid-bg:before{content:"";position:absolute;inset:-10%;background:conic-gradient(from 0deg,transparent 0deg,rgba(168,85,247,.12) 65deg,transparent 130deg,rgba(62,166,255,.10) 210deg,transparent 290deg);filter:blur(50px);animation:ambientSpin 18s linear infinite}
...
.login-card-glow{position:absolute;inset:-1px;border-radius:23px;background:conic-gradient(from 180deg,rgba(168,85,247,.0),rgba(168,85,247,.55),rgba(62,166,255,.35),rgba(168,85,247,.0));filter:blur(14px);opacity:.18;z-index:-1;animation:cardGlow 8s linear infinite}@keyframes cardGlow{to{transform:rotate(360deg)}}
...
.badge-glow{position:absolute;inset:-14px;border-radius:50%;background:radial-gradient(circle,rgba(168,85,247,.55),transparent 70%);filter:blur(10px);animation:pulseGlow 2.6s ease-in-out infinite}
  ...
  <div class="card">
    ...
    <form method="post" action="/login" id="loginForm">
      <div class="field">
        ...
        <div class="inp">
          <input type="text" name="username" id="username" data-i18n-placeholder="emailPh" autocomplete="username" autofocus>
...
:root:not([data-theme]) .topbar{height:68px;padding:0 26px;background:rgba(13,17,27,.88);backdrop-filter:blur(18px);box-shadow:0 1px 0 rgba(255,255,255,.025)}
[data-theme="light"] .topbar{height:68px;padding:0 26px;background:rgba(255,255,255,.88);backdrop-filter:blur(18px);box-shadow:0 1px 0 rgba(30,34,60,.04)}
    ...
    <div class="page" id="pg-settings">
      ...
      <div class="two-col">
        <div class="card settings-card">
          ...
          <button class="btn primary" onclick="changePassword()"><i class="ti ti-key"></i>تغییر امن رمز</button>
        </div>
        <div class="card settings-card">
...
function gotoPage(pg){
  ...
  const loaders = {overview:refreshOverview, links:loadLinks, categories:loadCategories, subgroups:loadSubGroups,
    plans:loadPlans, reports:loadReports, admins:loadAdmins, activity:loadActivity, messages:loadMessages, settings:loadSettings};
...
function getDashLang(){ try{ return localStorage.getItem('vw_dash_lang') || 'fa'; }catch(e){ return 'fa'; } }
function applyDashLang(lang){
  const d = DASH_I18N[lang] || DASH_I18N.fa;
  document.querySelectorAll('#tabnav [data-i18n]').forEach(el=>{
...
function updatePageTitle(pg){
  const d = DASH_I18N[getDashLang()] || DASH_I18N.fa;
  const mainEl = document.getElementById('pageTitleMain');
  const subEl = document.getElementById('pageTitleSub');
  if(mainEl) mainEl.textContent = d['nav_'+pg] || pg;
  if(subEl) subEl.textContent = d['pt_'+pg] || '';
}
...
let linksTimer=setInterval(()=>{if(CURRENT_PAGE==='links') loadLinks(true)},5000);
...
function ibRowHtml(l){
  ...
  return `<tr class="${l.active?'':'ib-row-off'}" data-uid="${l.uuid}">
    ...
    <td><div class="ib-clientcell"><button class="iconbtn" title="مدیریت کلاینت‌ها" onclick="openClients('${l.uuid}')"><i class="ti ti-users"></i></button><span>${Number(l.client_count||0)} کلاینت</span></div><div style="font-size:9px;color:var(--sub2);margin-top:4px">اتصال زنده: ${Number(l.connected_ips||0)} / ${Number(l.connection_limit||0)||'∞'} · IP: ${Number(l.ip_limit||0)||'∞'}</div></td>
...
function manualBuilderHtml(l){
  ...
  const conn = Number(l.connection_limit||0);
  const speed = Number(l.speed_limit_bytes||0);
  const speedMbit = speed ? Math.max(1, Math.round(speed*8/1000000)) : '';
  return `
    <div class="inbound-builder">
      <div class="ib-hero">
        <div class="ib-title"><div class="ib-icon"><i class="ti ti-adjustments-horizontal"></i></div><div><b>${l.uuid?'ویرایش اینباند':'ساخت اینباند حرفه‌ای'}</b><small>ساختار مرحله‌ای برای پروتکل، انتقال، امنیت و محدودیت‌ها</small></div></div>
        <span class="badge gray">ADVANCED</span>
      </div>

      <div class="ib-section">
        <div class="ib-section-head"><div><b>۱. مشخصات اصلی</b><small>شناسه و سیاست مصرف</small></div><i class="ti ti-id"></i></div>
        <div class="ib-section-body">
          <div class="ib-grid">
            ...
            <div class="grp ib-full"><label>یادداشت داخلی</label><input id="fNote" value="${escapeHtml(l.note||'')}" placeholder="توضیحات اختیاری برای مدیریت"></div>
          </div>
        </div>
      </div>

      <div class="ib-section">
        <div class="ib-section-head"><div><b>۲. پروتکل و انتقال</b><small>پایه اتصال را انتخاب کنید</small></div><i class="ti ti-network"></i></div>
        <div class="ib-section-body">
          <div class="ib-grid">
            <div class="grp"><label>پروتکل</label><select id="mBase" onchange="onManualChange();updateBuilderSummary()">${baseOpts}</select></div>
            <div class="grp"><label>Network / Transport</label><select id="mNetwork" onchange="onManualChange();updateBuilderSummary()">${netOpts}</select></div>
            <div class="grp"><label>Security</label><select id="mSecurity" onchange="onManualChange();updateBuilderSummary()">${secOpts}</select></div>
            <div class="grp"><label>Fingerprint</label><select id="mFingerprint" onchange="updateBuilderSummary()">${fpOpts}</select></div>
          </div>
      ...
      <div class="ib-section">
        <div class="ib-section-head"><div><b>۳. Endpoint و پورت</b><small>آدرس مقصد و پارامترهای انتقال</small></div><i class="ti ti-server-2"></i></div>
        <div class="ib-section-body">
          <div class="ib-grid">
            <div class="grp"><label>آدرس / دامنه</label><div class="endpoint-input"><input id="mAddress" placeholder="example.com یا Railway TCP Proxy" value="${escapeHtml(l.address||'')}" oninput="updateBuilderSummary()"><button type="button" class="btn endpoint-btn" onclick="loadRailwayEndpoint()" title="استفاده از TCP Proxy ریل‌وی"><i class="ti ti-cloud"></i>Railway</button></div></div>
            <div class="grp"><label>Port</label><div class="endpoint-input"><input id="mPort" type="number" min="1" max="65535" value="${l.port||443}" oninput="updateBuilderSummary()"><button type="button" class="btn endpoint-btn" onclick="testCurrentTcp()"><i class="ti ti-activity"></i> تست پینگ</button></div></div>
            ...
            <div id="mXhttpModeWrap" class="grp ib-full"><label>XHTTP Mode</label><select id="mXhttpMode">${xhttpOpts}</select></div>
            <div id="mGrpcWrap" class="ib-grid ib-full">
              <div class="grp"><label>Service Name</label><input id="mGrpcService" placeholder="service" value="${escapeHtml(l.grpc_service_name||'')}"></div>
              <div class="grp"><label>gRPC Mode</label><select id="mGrpcMode"><option value="gun" ${(l.grpc_mode||'gun')==='gun'?'selected':''}>gun</option><option value="multi" ${l.grpc_mode==='multi'?'selected':''}>multi</option></select></div>
            </div>
            <div id="mTcpWrap" class="ib-grid ib-full">
              <div class="grp"><label>Header Type</label><select id="mHeaderType"><option value="" ${!l.header_type?'selected':''}>none</option><option value="http" ${l.header_type==='http'?'selected':''}>http</option></select></div>
              <div class="grp"><label>Flow</label><select id="mFlow"><option value="" ${!l.flow?'selected':''}>—</option><option value="xtls-rprx-vision" ${l.flow==='xtls-rprx-vision'?'selected':''}>xtls-rprx-vision</option></select></div>
            </div>
          </div>
        </div>
      </div>

      <div id="mTlsWrap" class="ib-section">
        <div class="ib-section-head"><div><b>۴. TLS</b><small>تنظیمات امنیت انتقال</small></div><i class="ti ti-lock"></i></div>
        <div class="ib-section-body"><div class="ib-grid">
          <div class="grp"><label>SNI</label><input id="mSni" placeholder="domain.com" value="${escapeHtml(l.sni||'')}"></div>
          <div class="grp"><label>ALPN</label><input id="mAlpn" placeholder="h2,http/1.1" value="${escapeHtml(l.alpn||'')}"></div>
          <label class="chk ib-full"><input type="checkbox" id="mAllowInsecure" ${l.allow_insecure?'checked':''}> Allow Insecure</label>
        </div></div>
      </div>

      <div id="mRealityWrap" class="ib-section">
        <div class="ib-section-head"><div><b>۴. Reality</b><small>پارامترهای Reality برای نود Xray</small></div><i class="ti ti-key"></i></div>
        <div class="ib-section-body"><div class="ib-grid">
          <div class="grp"><label>SNI / Dest</label><input id="mRealitySni" placeholder="www.example.com" value="${escapeHtml(l.sni||'')}"></div>
          <div class="grp"><label>Short ID</label><input id="mRealitySid" value="${escapeHtml(l.reality_short_id||'')}"></div>
          <div class="grp ib-full"><label>Public Key</label><input id="mRealityPbk" value="${escapeHtml(l.reality_public_key||'')}"></div>
          <div class="grp"><label>SpiderX</label><input id="mRealitySpx" value="${escapeHtml(l.reality_spider_x||'/')}" ></div>
          <div class="grp" style="display:flex;align-items:end"><button type="button" class="btn" style="width:100%" onclick="generateRealityKeys()"><i class="ti ti-key"></i> تولید کلید</button></div>
        </div><p class="ib-help">کلید خصوصی فقط برای نود Xray شماست و در داده‌های پنل ذخیره نمی‌شود.</p></div>
      </div>

      <div class="ib-section">
        <div class="ib-section-head"><div><b>۵. محدودیت و مصرف</b><small>کنترل ترافیک، سرعت و اتصال</small></div><i class="ti ti-chart-dots"></i></div>
        <div class="ib-section-body">
          <div class="ib-grid three">
            <div class="grp"><label>حجم</label><input id="fLimitVal" type="number" min="0" placeholder="0 = نامحدود" value="${l.limit_bytes?Math.round(l.limit_bytes/1073741824):''}"></div>
            <div class="grp"><label>واحد</label><select id="fLimitUnit"><option value="GB">GB</option><option value="MB">MB</option></select></div>
            <div class="grp"><label>اعتبار (روز)</label><input id="fDays" type="number" min="0" placeholder="0 = نامحدود" value="${l.expires_at ? Math.max(0, Math.ceil((new Date(l.expires_at).getTime()-Date.now())/86400000)) : ''}"></div>
            <div class="grp"><label>IP Limit</label><input id="fIpLimit" type="number" min="0" value="${l.ip_limit||0}" placeholder="0 = نامحدود"></div>
            <div class="grp"><label>Connection Limit</label><input id="fConnLimit" type="number" min="0" value="${conn}" placeholder="0 = نامحدود"></div>
            <div class="grp"><label>تعداد خروجی کانفیگ</label><input id="fConfigCount" type="number" min="1" max="40" value="${l.config_count||1}"></div>
      ...
      <div class="ib-section">
        <div class="ib-section-head"><div><b>خلاصه اینباند</b><small>قبل از ذخیره بررسی کنید</small></div></div>
        <div class="ib-section-body"><div class="ib-summary">
          <div class="sum"><small>PROTOCOL</small><b id="sumProtocol">—</b></div>
          <div class="sum"><small>NETWORK</small><b id="sumNetwork">—</b></div>
          <div class="sum"><small>SECURITY</small><b id="sumSecurity">—</b></div>
          <div class="sum"><small>PORT</small><b id="sumPort">—</b></div>
          <div class="sum"><small>TRAFFIC</small><b id="sumTraffic">—</b></div>
          <div class="sum"><small>LIMITS</small><b id="sumLimits">—</b></div>
        </div></div>
      </div>
    </div>`;
}
...
function updateBuilderSummary(){
  ...
  set('sumProtocol', val('mBase')); set('sumNetwork', val('mNetwork')); set('sumSecurity', val('mSecurity')); set('sumPort', val('mPort'));
  const traffic=Number(val('fLimitVal'))||0, unit=val('fLimitUnit');
  set('sumTraffic', traffic ? `${traffic} ${unit}` : 'نامحدود');
  const ip=Number(val('fIpLimit'))||0, conn=Number(val('fConnLimit'))||0;
  set('sumLimits', `IP ${ip||'∞'} / CONN ${conn||'∞'}`);
}

function onManualChange(){
  const net = $('mNetwork')?.value || 'ws';
  const sec = $('mSecurity')?.value || 'tls';
  const show=(id,yes)=>{const e=$(id);if(e)e.style.display=yes?'':'none'};
  show('mWsXhttpWrap', net==='ws'||net==='xhttp');
  show('mXhttpModeWrap', net==='xhttp');
  show('mGrpcWrap', net==='grpc');
  show('mTcpWrap', net==='tcp');
  show('mTlsWrap', sec==='tls');
  show('mRealityWrap', sec==='reality');
  const hint=$('mLiveHint');
  if(hint){
    const live=(MANUAL_META.live_combos||[]).some(c=>c[0]===net && c[1]===sec);
    hint.className='ib-status '+(live?'ok':'warn');
    hint.innerHTML=live?'<i class="ti ti-circle-check"></i> این ترکیب توسط هسته فعلی پنل پشتیبانی می‌شود.':'<i class="ti ti-alert-triangle"></i> این ترکیب فقط لینک را می‌سازد و برای سرو واقعی به نود Xray-core نیاز دارد.';
  }
...
function onProtocolModeChange(){
  ...
  if(isManual) setTimeout(()=>{onManualChange();updateBuilderSummary()},0);
}

function openLinkDrawer(uid){
  const editing=!!uid, l=editing?(LINKS.find(x=>x.uuid===uid)||{}):{};
  const isManual=l.protocol==='manual';
  const protoOpts=PROTOCOLS.map(p=>`<option value="${p.id}" ${(!isManual&&l.protocol===p.id)?'selected':''}>${p.functional?'✓ ':'! '}${p.label}</option>`).join('')+`<option value="manual" ${isManual?'selected':''}>دستی / حرفه‌ای</option>`;
  ...
  openDrawer(editing?'ویرایش اینباند':'اینباند جدید', `
    <div class="grp"><label>حالت ساخت</label><select id="fProtocol" onchange="onProtocolModeChange()">${protoOpts}</select></div>
    ...
    <div id="quickFieldsWrap" style="display:${isManual?'none':'block'}">
      <div class="ib-section"><div class="ib-section-head"><div><b>مشخصات سریع</b><small>برای ساخت کانفیگ آماده</small></div></div><div class="ib-section-body">
        <div class="ib-grid"><div class="grp"><label>نام / Remark</label><input id="qLabel" value="${escapeHtml(l.label||'')}" placeholder="مثلاً: VIP-01"></div>
        <div class="grp"><label>دسته‌بندی</label><select id="qCategory"><option value="0">بدون دسته</option>${CATEGORIES.map(c=>`<option value="${c.id}" ${String(l.category_id)===String(c.id)?'selected':''}>${escapeHtml(c.name)}</option>`).join('')}</select></div>
        <div class="grp"><label>Port</label><input id="fPort" type="number" min="1" max="65535" value="${l.port||443}"></div>
        <div class="grp"><label>Fingerprint</label><select id="fFingerprint">${fpOptsQuick}</select></div>
        <div class="grp ib-full"><label>ALPN</label><input id="fAlpn" value="${escapeHtml(l.alpn||'')}" placeholder="h2,http/1.1"></div>
        <div class="grp"><label>حجم (GB)</label><input id="qLimitVal" type="number" min="0" value="${l.limit_bytes?Math.round(l.limit_bytes/1073741824):''}"></div>
        <div class="grp"><label>اعتبار (روز)</label><input id="qDays" type="number" min="0" value="${l.expires_at?Math.max(0,Math.ceil((new Date(l.expires_at).getTime()-Date.now())/86400000)):''}"></div>
        <div class="grp"><label>IP Limit</label><input id="qIpLimit" type="number" min="0" value="${l.ip_limit||0}"></div>
        <div class="grp"><label>Connection Limit</label><input id="qConnLimit" type="number" min="0" value="${l.connection_limit||0}"></div>
        <div class="grp"><label>تعداد کاربر اینباند</label><input id="fClientLimit" type="number" min="0" max="1000" value="${l.client_limit||0}" placeholder="0 = نامحدود"></div>
        ...
        <div class="grp"><label>Speed (Mbit/s)</label><input id="qSpeed" type="number" min="0" value="${l.speed_limit_bytes?Math.max(1,Math.round(l.speed_limit_bytes*8/1000000)):''}"></div>
        <div class="grp ib-full"><label>یادداشت</label><input id="qNote" value="${escapeHtml(l.note||'')}"></div></div>
      </div></div>
    </div>`,
...
async function submitLink(uid){
  ...
  if(!proto){toast('پروتکل مشخص نشده است',false);return}
  const isManual=proto==='manual';
  // نکته مهم: فرم «دستی/حرفه‌ای» و فرم «سریع» هر دو همزمان در DOM هستند (یکی مخفی)،
  // پس شناسه‌های این دو فرم دیگر مشترک نیستند تا document.getElementById همیشه
  // مقدار فرم درستِ فعال را بخواند، نه فرم مخفیِ دیگر را (باگ قبلی همین بود).
  const idFor=(name)=>{
    const manualMap={label:'fLabel',category:'fCategory',limit:'fLimitVal',days:'fDays',ip:'fIpLimit',conn:'fConnLimit',speed:'fSpeed',note:'fNote'};
    const quickMap ={label:'qLabel',category:'qCategory',limit:'qLimitVal',days:'qDays',ip:'qIpLimit',conn:'qConnLimit',speed:'qSpeed',note:'qNote'};
    return (isManual?manualMap:quickMap)[name];
  };
  const portEl=isManual?$('mPort'):$('fPort'); const port=Number(portEl?.value||443);
  if(!Number.isInteger(port)||port<1||port>65535){toast('پورت باید بین 1 تا 65535 باشد',false);return}
  const num=(id)=>Math.max(0,Number($(id)?.value||0));
  const body={label:($(idFor('label'))?.value||'').trim(),protocol:proto,category_id:$(idFor('category'))?.value||'0',limit_value:num(idFor('limit')),limit_unit:$('fLimitUnit')?.value||'GB',expires_days:num(idFor('days')),expires_at:$('fExpiresAt')?.value||'',ip_limit:num(idFor('ip')),connection_limit:num(idFor('conn')),client_limit:num('fClientLimit'),config_count:Math.max(1,Math.min(40,num('fConfigCount')||1)),speed_limit_value:num(idFor('speed')),speed_limit_unit:'MBIT',note:($(idFor('note'))?.value||'').trim()};
  if(isManual){
    const sec=$('mSecurity').value;
    body.port=port; body.fingerprint=$('mFingerprint').value; body.alpn=$('mAlpn')?.value||'';
    body.manual={base_protocol:$('mBase').value,network:$('mNetwork').value,security:sec,address:$('mAddress')?.value||'',path:$('mPath')?.value||'',host_header:$('mHost')?.value||'',sni:sec==='reality'?($('mRealitySni')?.value||''):($('mSni')?.value||''),alpn:$('mAlpn')?.value||'',flow:$('mFlow')?.value||'',grpc_service_name:$('mGrpcService')?.value||'',grpc_mode:$('mGrpcMode')?.value||'gun',xhttp_mode:$('mXhttpMode')?.value||'auto',header_type:$('mHeaderType')?.value||'',allow_insecure:!!$('mAllowInsecure')?.checked,reality_public_key:$('mRealityPbk')?.value||'',reality_short_id:$('mRealitySid')?.value||'',reality_spider_x:$('mRealitySpx')?.value||'/'};
  }else{body.port=port;body.fingerprint=$('fFingerprint')?.value||'chrome';body.alpn=$('fAlpn')?.value||'';}
...
messageTimer=setInterval(()=>{if(CURRENT_PAGE==='messages') loadMessages()},10000);
